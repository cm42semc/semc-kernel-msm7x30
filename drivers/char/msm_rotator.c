...
static int get_img(struct msmfb_data *fbd,
	...
	int *p_need, unsigned int secure)
{
	int ret = 0;
#ifdef CONFIG_FB
...
static void put_img(struct file *p_file, unsigned int secure)
{
#ifdef CONFIG_ANDROID_PMEM
	if (p_file != NULL)
...
static int msm_rotator_do_rotate(unsigned long arg)
{
	...
	rc = get_img(&info.src, (unsigned long *)&in_paddr,
			(unsigned long *)&src_len, &srcp0_file, &ps0_need, 0);
	if (rc) {
	...
	if (((info.version_key & VERSION_KEY_MASK) == 0xA5B4C300) &&
		...
		rc = get_img(&info.src_chroma,
				...
				(unsigned long *)&src_len, &srcp1_file, &p_need, 0);
		if (rc) {
		...
		rc = get_img(&info.dst_chroma,
				...
				(unsigned long *)&dst_len, &dstp1_file, &p_need, img_info->secure);
		if (rc) {
